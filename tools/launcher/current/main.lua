...
-- Kept behind the actual Red/Blue/Yellow handoff below: preparation stays on
-- its proven 800x600 surface. WebViewport owns the post-init SDL/LÖVE handshake
-- and never lets the DOM canvas become an independent geometry authority.
local WebViewport = require("src.core.WebViewport")
local function configureWebWidescreen()
  return WebViewport.configure()
end
...
local function bootGame(version, variant)
  ...
  -- launcher's touch coordinate space on Safari.  Resize only at the actual
  -- game handoff, before Game:load allocates render targets.
  configureWebWidescreen()
...
function love.load(args)
  ...
  -- The outer PWA locates the private LÖVE save directory through this tiny
  -- marker before delivering a locally selected ROM into the Emscripten
  -- filesystem.  It contains no path, ROM byte or user data.
  if Platform.isWeb() then
    pcall(love.filesystem.write, "web-bridge-ready.txt", "gen1recomp-web-v1")
  end
...
function love.mousepressed(x, y, button, istouch)
  ...
  if TouchEditor then
    ...
    if love.system.getOS() == "Android" or Platform.isWeb() then return end
  ...
  if Importer then
    ...
    if istouch and (love.system.getOS() == "Android"
        or love.system.getOS() == "iOS" or Platform.isWeb()) then return end
  ...
  if editorMode and EditorApp.mousepressed then
    ...
    if istouch and (love.system.getOS() == "Android" or Platform.isWeb()) then
      return
    end
...
function love.run()
  ...
  -- love.js/Emscripten already schedules the main loop with requestAnimationFrame
  -- when vsync=1 (conf.lua). Sleeping inside that callback makes Safari miss the
  -- next RAF and produces the 20-45 FPS cadence seen in the iPhone traces even
  -- when Lua draw/update work is comfortably below 16.7 ms. Native builds still
  -- use the explicit FrameCap below; Web returns to the browser immediately and
  -- lets RAF/vsync own presentation pacing.
  local webRafPaced = paced and love and love.system and love.system.getOS
  ...
  -- Web uses RAF as the scheduler, but MAX FPS remains a render cap. This
  -- deadline skips presentation callbacks when the requested cap is below the
  -- display refresh rate instead of blocking inside RAF with love.timer.sleep.
  local webNextPresent = nextFrame
  local dt = 0
  ...
  return function()
    ...
    local renderThisFrame = true
    if webRafPaced and love.timer then
      local now = love.timer.getTime()
      local budget = 1 / FrameCap.current
      if now + 0.00025 < webNextPresent then
        renderThisFrame = false
      else
        webNextPresent = webNextPresent + budget
        -- Re-anchor after a stall instead of issuing a burst of presents.
        if now - webNextPresent > budget then
          webNextPresent = now + budget
        end
      end
    end

    if renderThisFrame and love.graphics and love.graphics.isActive() then
    ...
    if love.timer then
      if webRafPaced then
        -- No sleep here: the Emscripten main-loop scheduler is already RAF.
        -- Returning promptly is what allows Safari to schedule the next vsync.
      elseif paced then
